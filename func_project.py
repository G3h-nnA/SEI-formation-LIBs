    ...
    for param in old_set:
        try:
            if old_set[param] != pybamm.ParameterValues("ORegan2022")[param]:
                print(f"For {param}, Chen2020 value:{old_set[param]} \tORegan2022 value: {pybamm.ParameterValues("ORegan2022")[param]}\n")
                old_set.update({param: pybamm.ParameterValues("ORegan2022")[param]})
            else:
                print(f"Skipped. The values for ({param}) is the same for both...")
        except:
            print(f"There is no value for {param} in ORegan2022")
